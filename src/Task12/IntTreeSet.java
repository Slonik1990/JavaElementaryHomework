...
public class IntTreeSet implements Set {
    ...
    class TreeIterator implements Iterator {
        ...
        public Object next() {
            ...
            //итератор начинает свой путь с минимального элемента, у которого однозначно нет левой ветки
            ...
            if(current.hasRight()){
                ...
                //если у текущей ноды нет правой ветки, а левая обработана изначально, либо отсутствует указатель возвращается к родителю
                //делается минимум один откат к родителю, а далее проверяется не вызывалась ли данная нода ранее
                do{
                    current = current.getParent();
                    //если попытки найти родителя со значением большим, чем уже обработанное, ничего не дали
                    //и произошло обращение к несуществующему родителю корня - дерево полностью проработанно
                    //current зафиксируется как null, что полностью соответствует логике программы
                    //при следующем вызове hasNext()вернет false
                    if(current == null){
                        break;
                    }
                }
