package Task15;
/**
 * Основан на массиве содержащем элеиенты ключ-значение
 * Не может содержать одинаковых ключей
 * Имеет коэффициент загрузки, для того, чтобы связные списки не растягивались чрезмерно и ухудшали производительность
 * Ключ не может быть null
 * Значение может быть null
 * индекс вычисляется на основе хэш кода
 * новые элементыпри занятой ячейке по вычисленному индексу вставляются в начало цепочки
 */
